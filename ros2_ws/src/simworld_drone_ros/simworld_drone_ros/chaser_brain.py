...
class ChaserBrain(Node):
    ...
    def _build_ollama_request(self, distance: float | None, target_visible: bool, target_mode: str) -> dict:
        ...
        system = (
            ...
            "Valid strategies: herd_to_boundary, shadow_until_close, fake_left_cut_right, predict_and_camp, deny_center, spiral_search. "
            "Valid tactics: chase_direct, intercept, cutoff_left, cutoff_right, pressure, search_last_seen. "
    ...
    def _apply_heat_limit(self, tactic: str, speed: float) -> float:
        ...
        if self.heat >= self.heat_max * 0.85 and tactic in ("intercept", "cutoff_left", "cutoff_right", "finish_commit"):
    ...
    def _choose_tactic(self, distance: float | None) -> str:
        ...
        if self.heat >= self.heat_max * 0.85 and tactic in ("intercept", "cutoff_left", "cutoff_right"):
            tactic = "pressure"
    ...
    def _desired_altitude_for_tactic(self, tactic: str) -> float:
        ...
        if tactic in ("intercept", "finish_commit", "chase_direct"):
