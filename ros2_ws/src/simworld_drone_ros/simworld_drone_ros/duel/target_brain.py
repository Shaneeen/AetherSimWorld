...
class TargetBrain(Node):
    def __init__(self) -> None:
        ...
        self.arena_boundary_margin = max(0.0, self._read_float_env("SIM_ARENA_BOUNDARY_MARGIN_CM", 90.0))
        ...
        self.altitude_tactic_hold_sec = self._read_float_env("SIM_TARGET_ALTITUDE_TACTIC_HOLD_SEC", 2.5)
        ...
        self.commit_escape_range_cm = self._read_float_env("SIM_TARGET_COMMIT_ESCAPE_RANGE_CM", 1150.0)
        self.safe_distance_cm = self._read_float_env("SIM_TARGET_SAFE_DISTANCE_CM", 1850.0)
    ...
    def _near_arena_wall(self) -> bool:
        wall_x = max(0.0, self.bounds_x - self.arena_boundary_margin)
        wall_y = max(0.0, self.bounds_y - self.arena_boundary_margin)
        return abs(self.current_x) >= wall_x - 35.0 or abs(self.current_y) >= wall_y - 35.0
    ...
    def _update_stuck_state(self) -> bool:
        ...
        near_wall = self._near_arena_wall()
        barely_moving = self.actual_speed_cm_s <= self.stuck_speed_threshold_cm_s or (
            near_wall and self.actual_speed_cm_s <= self.stuck_speed_threshold_cm_s * 1.25
        )
        if should_be_moving and barely_moving:
            if self.stuck_started_at is None:
                self.stuck_started_at = now
            elif now - self.stuck_started_at >= (0.3 if near_wall else self.stuck_after_sec):
    ...
    def _route_escape_heading(
        ...
        if not self.tactical_blockers:
            return hx, hy, None
        desired_x = self.current_x + hx * max(self.current_speed, self.evade_speed) * 2.5
        desired_y = self.current_y + hy * max(self.current_speed, self.evade_speed) * 2.5
        if prefer_cover and ref_x is not None and ref_y is not None:
            cover = cover_point(
                (self.current_x, self.current_y, self.current_z),
                (ref_x, ref_y, self.current_z),
            ...
            if cover is not None:
                ...
                hx, hy = self._normalize_heading(routed_x - self.current_x, routed_y - self.current_y)
                return hx, hy, cover[2] if route_reason is None else f"{cover[2]}; {route_reason}"
        routed_x, routed_y, route_reason = route_around_blockers(
            (self.current_x, self.current_y, self.current_z),
            (desired_x, desired_y),
            self.tactical_blockers,
    ...
    def _choose_altitude_tactic(self, threat_state: str, visible: bool) -> str:
        ...
        if self.current_z >= self.max_z - 35.0:
            return "dive_escape"
        if (
            self.altitude_tactic in {"climb_escape", "dive_escape", "level_escape"}
            and now - self.altitude_tactic_at < self.altitude_tactic_hold_sec
            and abs(self.target_z - self.current_z) > 20.0
        ):
            return self.altitude_tactic
    ...
    def _set_altitude_target_for_tactic(self, tactic: str) -> None:
        if tactic != self.altitude_tactic:
            self.altitude_tactic_at = time.time()
    ...
    def _fallback_escape_heading(self, visible: bool, threat_state: str) -> tuple[float, float, str]:
        ...
        if not visible:
            ...
            hx, hy, route_reason = self._route_escape_heading(hx, hy, ref_x, ref_y, prefer_cover=True)
            if self._recent_memory_available():
                tactic = "hidden_from_memory"
            else:
                tactic = "random_hidden"
            if route_reason:
                tactic += "_cover"
            return hx, hy, tactic

        hx, hy = self._normalize_heading(away_x, away_y)
        hx, hy, route_reason = self._route_escape_heading(hx, hy, ref_x, ref_y, prefer_cover=threat_state != "patrol")
        if route_reason:
            return hx, hy, "cover_escape"
    ...
    def _build_ollama_request(
        ...
        system = (
            ...
            "When threat_state is panic or distance_cm is below safe_distance_cm, prioritize increasing separation over juking in place. "
    ...
    def _heading_from_tactic(self, tactic: str, ref_x: float | None, ref_y: float | None) -> tuple[float, float, str]:
        ...
        if tactic in ("zigzag_left", "zigzag_right"):
            ...
            hx, hy, route_reason = self._route_escape_heading(hx, hy, ref_x, ref_y, prefer_cover=False)
            if route_reason:
                return hx, hy, f"{tactic}_route"
            return hx, hy, tactic
        ...
        hx, hy, route_reason = self._route_escape_heading(hx, hy, ref_x, ref_y, prefer_cover=False)
        if route_reason:
            return hx, hy, f"{tactic or 'flee_direct'}_route"
    ...
    def _choose_tactic(self, visible: bool, distance: float | None, threat_state: str) -> tuple[float, float, str]:
        ...
        if ref_x is not None and ref_y is not None:
            closing_fast = self.last_distance_trend < -80.0
            too_close = distance is not None and distance <= self.commit_escape_range_cm
            below_safe_visible = visible and distance is not None and distance < self.safe_distance_cm
            if threat_state == "panic" or (too_close and closing_fast):
                tactic = "burst_escape" if self._can_burst() or time.time() < self.burst_until else "flee_direct"
                hx, hy, tactic = self._heading_from_tactic(tactic, ref_x, ref_y)
                self.cached_strategy = "tempo_change" if tactic.startswith("burst") else "keep_distance"
                self.cached_tactic = tactic
                self.cached_tactic_at = time.time()
                return hx, hy, tactic
            if below_safe_visible and self.cached_tactic in {"random_escape", "juke_left", "juke_right", "zigzag_left", "zigzag_right"}:
                tactic = "flee_direct" if self.last_distance_trend >= -35.0 else "burst_escape"
                hx, hy, tactic = self._heading_from_tactic(tactic, ref_x, ref_y)
                self.cached_strategy = "keep_distance" if tactic == "flee_direct" else "tempo_change"
                self.cached_tactic = tactic
                self.cached_tactic_at = time.time()
                return hx, hy, tactic
